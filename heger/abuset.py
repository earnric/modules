...
class AbuRat():
    ...
    pattern = re.compile(r'[ANZ][:=][0-9]+|[A-Z][a-z]?-?[0-9]*|[a-z]{1,2}-?[0-9]*')
    ...
    def __getitem__(self, key):
        ...
        for consntence, whereas tuple length different from 1 could be
        treated correctly - and differnetly from ambiguous 2-tuples,
        we just raise an error here.
        """
        if isinstance(key, tuple):
            assert len(key) == 2, """
            supporting only tuples of length 2 that are use for ratios;
    ...
    def __call__(self, *args):
        """
        This method is the same as '[..]' but can accepting tuples for
        grouping.
        """
        val = []
        ref = []
