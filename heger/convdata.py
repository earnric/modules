...
class ConvData(Logged):
    ...
    def aw(self):
        """
        central angular velocity (rad/s)
        """
    ...
    def awx(self):
        """
        central rotation invarian \tilde(\omega) (rad s**(-1) g**(2/3) cm**(-1))
...
class ConvRecord(object):
    ...
    def _load(self,
        ...
        if self.nvers < 10400:
            self._load_103xx(f,**kwargs)
        else:
            self._load_104xx(f,**kwargs)

        f.assert_eor()

    def _load_104xx(self,
                    f,
        ...
        if self.nvers >= 10500:
            if self.nconv > 0:
                ...
                if kwargs.get('fix_surf_10500', False):
                    if self.nconv >= 2:
                        # if ((self.convtype[-2] == 4 and self.convtype[-3] != 1)
                        #     or
                        #     (self.convtype[-2] == 3 and self.iconv[-3] == self.iconv[-2] - 1)):
                        if self.convtype[-2] in (3, 4):
                            self.convtype = np.insert(self.convtype, self.nconv - 1, 1)
    ...
    def _load_103xx(self,
        ...
        # just te be sure (others to be added later)
        assert 10100 <= self.nvers < 10400, 'DEBUG - {:g}'.format(self.nvers)
