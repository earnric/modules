...
class KepDump(Logged):
    ...
    def load_data(self, killburn = False):
        ...
        self.angj   = f.load_f8n([jmsave1])
        self.angdg  = f.load_f8n([jmsave1])
        self.angd1  = f.load_f8n([jmsave1])
        self.angd2  = f.load_f8n([jmsave1])
        self.angd3  = f.load_f8n([jmsave1])
        self.angd4  = f.load_f8n([jmsave1])
        self.angd5  = f.load_f8n([jmsave1])
        ...
        # magnetic field data
        #
        # To allow future physics, these field should also be written
        # out [0...jmsave1]
    ...
    def _write(self):
        ...
        f.write_f8(self.angj)
        f.write_f8(self.angdg)
        f.write_f8(self.angd1)
        f.write_f8(self.angd2)
        f.write_f8(self.angd3)
        f.write_f8(self.angd4)
        f.write_f8(self.angd5)
    ...
    def gamma(self):
        """
        Adiabatic exponent, d ln P / d ln rho, on zone boundary.
    ...
    def erot(self):
        ...
        """
        return 0.5 * self.angj * self.angw
    ...
    def angwwk(self):
        """
        shell angular velocity relative to keplerian
        """
        wwk = self.angw / (self.angwk + 1.e-99)
        return wwk
    ...
    def angw(self):
        """
        shell angular velocity (rad/sec)
        """
        aw = np.empty_like(self.angj)
        aw[1:-1] = self.angj[1:-1] / self.angi[1:-1]
        if 'angw0' in self.parm:
            aw[0] = self.parm.angw0
        else:
            aw[0] = np.nan
        aw[-1] = np.nan
        return aw
    ...
    def angwx(self):
        ...
        """
        s = slice(1,-1)
        awt = self.angjt[s] / self.angit[s]
        dnt = self.zm[s] * 3 / (4 * np.pi * self.rn[s]**3)
        awx = np.zeros_like(self.rn)
        awx[s] = awt * dnt**(-2 / 3)
        return awx
    ...
    def angjt(self):
        """
        interior total angular momentum (erg*sec)
        """
        s = slice(1,-1)
        angJ = np.zeros_like(self.xm)
        angJ[s] = self.angj[s] * self.xm[s]
        if 'angw0' in self.parm:
            angJ[0] = (self.parm.angw0 *
                      self.parm.summ0 *
                      calcai(0, self.rn[0]))
        angJ = angJ.cumsum()
        angJ[-1] = np.nan
    ...
    def angjtm(self):
        ...
        Note: could also use qparm.anglt
        """
        jtm = self.angjt
        jtm[1:-1] = 0.5 * (jtm[0:-2] + jtm[1:-1])
        return jtm
    ...
    def angabh(self):
        """
        black hole spin parameter (g)
        """
        g = physconst.Kepler.gee * self.parm.geemult
        a = np.zeros_like(self.xm)
        s = self.center_slice
        a[s] = ( self.angjtm[s] / self.zmm[s] *
    ...
    def angambh(self):
        """
        normalized black hole spin parameter a/m
        """
    ...
    class Core(collections.OrderedDict):
        class CoreData(object):
            __slots__ = ('j', 'zm_sun', 'rn', 'ybind', 'stot', 'angjt')
        ...
        head = " core            j      zm_sun     rn      ybind       stot      angjt"

        @classmethod
        def core_line(cls, name = None, core_data = None):
            if name is None:
                return cls.head
    ...
    @cachedmethod
    def core(self, xlim = 0.01, felim = 0.1):
        """
    ...
    def ertl(self):
        """
        Return two-parameter SN criterion, Ertl+ 2015, arXiv:1503.07522
...
def compdump(d1, d2, detail = True):
    ...
    def cmp_arr(name):
        ...
        v2 = d2.__getattribute__(name)
        if len(v1) != len(v2):
            print('{:s} have different length.'.format(name))
            return
        if not np.alltrue(v1 == v2):
            print('{:s} differs'.format(name))
            if detail:
