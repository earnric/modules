...
class KepAbuData(object):
    """
    KepIon and distribution information, e.g., structure.
    """
    # should be initialized with KepIon and abundace vector,
...
class KepAbuData(ClearCache):
    ...
    def netnum_ranges(self):
        ...
        for i in range(len(ii)-1):
            ranges += [(netnum[ii[i] + 1], np.array([ii[i] + 1, ii[i + 1]]))]
        if netnum[-1] == netnum[-2]:
            ranges += [(netnum[-2], np.array([ii[-1] + 1, len(netnum) - 1]))]
        return ranges
    ...
    def BE(self):
        ...
        """
        BE = np.ndarray(self.n)
        BE[:] = np.nan
        for n, r in self.netnum_ranges:
            ii = slice(r[0], r[1]+1)
            if n == 1:
                BE[ii] = np.dot(self.ppn[ii,0:19], self.BE_APPROX)
            else:
    ...
    def ME(self):
        ...
        """
        ME = np.ndarray(self.n)
        ME[:] = np.nan
        for n, r in self.netnum_ranges:
            ii = slice(r[0], r[1]+1)
            if n == 1:
                ME[ii] = np.dot(self.ppn[ii,0:19], self.ME_APPROX)
            else:
                ME[ii] = (
