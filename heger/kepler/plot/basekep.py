...
class BasePlotKep(BasePlot, Logged):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.showhead = kwargs.get('showhead', 'default')
    ...
    def clear(self):
        ...
        # this to move to frame
        self.fig.texts.clear()
        self.fig.legends.clear()

        super().clear()

    # maybe this needs to be part of a frame object?
    def header(self, *args, **kwargs):
        """
        Show header.  Later we may want to introduce differnt header types.
        """
        showhead = kwargs.get('showhead', self.showhead)
        if not showhead == 'default':
            return
        ...
        lines.append("Dc ={:11.4e}   Tc ={:11.4e}   Ln ={:11.4e}   Jm ={:5d}".format(
            ...
            qparm.xlumn,
            qparm.jm))
        if qparm.jm != parm.jmcalc:
            lines[-1] += "({:4d})".format(qparm.jmcalc)
        else:
            lines[-1] += "   Etot ={:11.4e}".format(qparm.ent)
