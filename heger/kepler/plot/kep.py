...
class Plot2(BasePlotKepAxes):
    ...
    def draw(self):
        ...
        if parm.vlimset > 0:
            ...
            self.ax.set_ylim(np.array([-1,1]) * parm.vlimset)
        elif parm.vlimset < 0:
            self.ax.set_yscale('linear')
...
class Plot78Base(BasePlotKepAxes):
    ...
    def draw(self):
        ...
        angj = data.angj[iim]
        ii = np.where(angj > 0)[0]
        if len(ii) > 0:
            angj0 = floor(log10(np.min(angj[ii])))
            angj1 = ceil(log10(np.max(angj[ii])))
        else:
            angj0 = -1
            angj1 = 1
        self.ax.plot(xm, angj, color = self.colors['angj'], label = r'$j$')

        angw = data.angw[iim]
        if len(ii) > 0:
            angw0 = floor(log10(np.min(angw[ii])))
            angw1 = ceil(log10(np.max(angw[ii])))
        else:
            angw0 = -1
            angw1 = 1
        wscale = round(0.5 * ((angw1 + angw0) - (angj1 + angj0)))
        self.ax.plot(xm, angw * 0.1**wscale, color = self.colors['angw'], label = r'$\omega$')

        # finally, set scales and labels
        ylim = np.array([min(angj0, angw0 - wscale), max(angj1, angw1 - wscale)])
        ylim = 10.**ylim
        self.ax.set_ylim(*ylim)
        ...
        if self.diffvar == 'nu':
            visc = parm.difim * (parm.angfjc * data.difi[iib] + data.angdg[iib] + data.bfvisc[iib]) * vunit
            difi = data.difi[iib] * parm.angfjc * vunit
            bfvisc = data.bfvisc[iib] * vunit
        else:
            visc = parm.difim * (data.difi[iib] + parm.angfc * data.angdg[iib] + data.bfdiff[iib]) * vunit
        ...
        for i in range(5):
            if self.diffvar == 'nu':
                ...
                diff = data.angd[iib,i] * parm.angfc * vunit
            if np.any(diff > 0):
...
class Plot6(BasePlotKepAxes):
    ...
    number of ison is stored in parm.numiso.
    """
...
class Plot3(BasePlotKepAxes):
    ...
    - iplotb (default parm.iplotb, see KEPLER doc):
      0: APPROX/QSE/NSE
      1: use BURN for APPROX
      2: use BURN above parm.bmasslo
      3: use BURN everywhere
     1x: plot BURN summed bu by element
     2x: plot BURN summed up by mass number

    - abunlim (default parm.abunlim)
    ...
    def draw(self):
        ...
        if iplotb1 in (1, 2, 3) and parm.inburn == 1:
            ppnb = data.ppnb
            ionnb = data.ionnb
            numib = data.numib
            ionsb = data.ionsb
            ...
            if iplotb1 == 1:
                ...
                llb = data.zm[0:jm] > parm.bmasslow
            else:
...
class Plot9(BasePlotKep):
    ...
    def draw(self):
        ...
        if parm.inburn == 0:
            raise Exception('[Plot 9] no BURN data')
        iblow = np.searchsorted(data.zm[0:jm], parm.bmasslow) + 1
        if iblow >= jm:
            raise Exception('[Plot 9] no BURN data')

        ipromin = parm.ipromin
        ipromax = parm.ipromax

        iprownd = parm.iprownd

        iproyld = self.kwargs.get('iproyld', parm.iproyld)

        minapro = parm.minapro
        maxapro = parm.maxapro

        proymax = parm.proymax
        proymin = parm.proymin

        proamax = parm.proamax
        proamin = parm.proamin

        profmax = parm.profmax
        profmin = parm.profmin
        ...
        if iproyld in (101, 102, 103):
            ...
            iisel = slice(max(iblow, ipromin), min(jm+1, ipromax))

            llb = np.tile(False, xm.shape)
            llb[iisel] = True
            ...
            for inb in range(len(numib)):
                ...
                if np.any(ll):
                    ...
                    mfrac = np.sum(mfrac * xm[np.newaxis, ll], axis=1)
        ...
        scale = min(ymax, 10**np.ceil(np.log10(np.max(mfrac))))
