...
def get_J_M_exp(series = 'z', 
    ...
    print('vvk(',series+mass_str[0],')=',dumps[0].angwwk[j0],', vvk(',series+mass_str[1],')=',dumps[1].angwwk[j1])
    ...
    print('d ln (J_ini/wwk) / d ln (M) = ', math.log((dumps[1].angwwk[j1]*dumps[0].qparm.anglint)/
                                            (dumps[0].angwwk[j0]*dumps[1].qparm.anglint))/(
            math.log(masses[0]/masses[1])))
    print('d ln (J/wwk) / d ln (M) = ', math.log((dumps[1].angwwk[j1]*dumps[0].qparm.anglt)/
                                            (dumps[0].angwwk[j0]*dumps[1].qparm.anglt))/(
            math.log(masses[0]/masses[1])))
    print('d ln (wwk) / d ln (M) = ', math.log((dumps[0].angwwk[j0])/
                                            (dumps[1].angwwk[j1]))/(
            math.log(masses[0]/masses[1])))
...
def get_J_Z_exp(series = ('z','u','v','t','o','s'), 
    ...
    for s,dump in zip(series, dumps):
        ...
        print(s + mass_str,':')
        j = dump.jm - 4
        print('vvk = ',dump.angwwk[j])
        print('J/J_ini = ',dump.qparm.anglt/dump.qparm.anglint)
        print('J_ini = ', dump.qparm.anglint)
        print('wwk = ', dump.angwwk[j])
        print('J/wwk = ', dump.qparm.anglt/dump.angwwk[j])
