...
def x7k():
    ...
    f = figure()
    plot(d.zm_sun, d.angwx, label = 'hign')
    plot(D.zm_sun, D.angwx, label = '7000')
    yscale('log')
    ...
    f = figure()
    plot(D.zm_sun, D.angj, label = 'hign')
    plot(d.zm_sun, d.angj, label = '7000')
    yscale('log')
    ...
    f = figure()
    plot(D.zm_sun, D.angjt, label = 'hign')
    plot(d.zm_sun, d.angjt, label = '7000')
    yscale('log')
    ylim(1.e43,1.e49)
    ...
    f = figure()
    plot(D.zm_sun, D.angw, label = 'hign')
    plot(d.zm_sun, d.angw, label = '7000')
    yscale('log')
    ...
    f = figure()
    plot(D.rn/D.rn[-1], D.angw/(2*pi)*1e9, label = 'hign')
    plot(d.rn/d.rn[-1], d.angw/(2*pi)*1e9, label = '7000')
    xscale('log')
    xlim(1.e-5,1.)
    yscale('log')
    xlabel('r/R')
    ylabel('rotation / nHz')
    plot(dm.rn/dm.rn[-1], dm.angw/(2*pi)*1e9, label = '6000')
    plot(dp.rn/dp.rn[-1], dp.angw/(2*pi)*1e9, label = '8000')
...
def seq():
    ...
    # f.canvas.get_parent_window().destroy()

    f = figure()
    for c,d in zip(colors, dumps):
        s = d.center_slice
        freq = d.angw[s]/(2* np.pi)*1.e9
