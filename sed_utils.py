...
The RAMSES format is a directory containing three files:
...
def convertS99toRamses(files=None, ZBins=None, Mtot=None, outDir=None):
    ...
    if files == None or ZBins == None:
        print(
            "Please write paths and metallicities (mass fraction",
    ...
    if Mtot == None:
        print(
            "Please write the stellar mass used in the S99, in units of",
    ...
    for file in files:  # Loop over SED tables for different metallicities
        ...
        for iAge in range(0, nAge):
            ...
            sedFile.write_record((10.0 ** lums) / 2e33 / Mtot)  # 2d33 = L_Sun
...
def plotRamsesSEDs(sedDir, iZ=0, iAge=None, pdf=None):
    ...
    Remember: indices are 1 based, NOT 0.
    """
    ...
    if iAge == None:  # Select some ages
        ...
        iAge = sorted(iAge)
    if np.isscalar(iAge):
        iAge = [iAge]  # if scalar argument was passed in
    cm = p.get_cmap("viridis")  # Curve colors
    NUM_COLORS = len(iAge)
    p.gca().set_prop_cycle(
...
def plotRamsesGroups(groupFiles=["SEDtable1.list"], isEgy=False, pdf=None):
    ...
    p.subplots_adjust(
        left=0.07, right=0.97, hspace=0.04, top=0.95, bottom=0.06, wspace=0.04
    )
    xrg = [2e-1, 2e4]
    lumrg = [1e39, 1e52]
    ...
    for i, fileName in enumerate(groupFiles):
        ...
        for iZ in range(0, nZ):
            ...
            ax.plot(
                data[iZ * nAge : (iZ + 1) * nAge, 0] * 1e3,
                data[iZ * nAge : (iZ + 1) * nAge, 4],
                label=label,
            )
        if i == 0:
            leg = ax.legend(frameon=False, prop={"size": 10})
        # Plot cross sections
...
def readRamsesSEDs(sedDir):
    ...
    print("Reading wave-spectra data")
    ...
    for iZ in range(0, nZ):
        for iAge in range(0, nAge):
            ...
            print(
                "Read spectrum for age {} and Z {} with len {}".format(
                    iAge, iZ, len(spectrum)
                )
            )
...
if __name__ == "__main__":
    ...
    if conv_BC03:  # Convert BX03 to ramses format
        files = [
            "Padova1994/chabrier/bc2003_lr_m22_chab_ssp.ised_ASCII",
